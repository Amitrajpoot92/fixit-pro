...
import { 
  ...
  Loader2, X, ArrowRight, ShieldCheck, User, Phone, Mail, AlertTriangle 
} from 'lucide-react';
...
export default function Tasks() {
  ...
  const getStatusBadgeColor = (status) => {
    ...
    if (s === 'cancelled') return 'bg-red-500/10 text-red-400 border-red-500/30'; // 🚀 Added Cancelled Badge
  ...
  // Filter Logic with 'Cancelled' tab
  const newRequests = tasks.filter(t => t.technicianStatus === 'Pending' && t.status !== 'Cancelled');
  const activeTasks = tasks.filter(t => t.technicianStatus === 'Accepted' && t.status !== 'Completed' && t.status !== 'Cancelled');
  const completedTasks = tasks.filter(t => t.status === 'Completed');
  const cancelledTasks = tasks.filter(t => t.status === 'Cancelled'); // 🚀 New filter
      ...
      <div className="max-w-6xl mx-auto pb-12 p-2 sm:p-4">
        ...
        <div className="mb-6 sm:mb-8">
          <h2 className="text-2xl sm:text-3xl font-black text-white flex items-center gap-2.5">
            <ClipboardList className="w-7 h-7 sm:w-8 h-8 text-emerald-500" /> Task Management
          </h2>
          <p className="text-slate-400 text-xs sm:text-sm mt-1">Manage orders, contact customers, and update live tracking status.</p>
        </div>

        {/* 🗂️ RESPONSIVE TABS (Added Cancelled) */}
        ...
        <div className="space-y-4 sm:space-y-6">
          {loadingTasks ? (
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl p-8 sm:p-12 text-center shadow-xl">
              ...
              <p className="text-xs sm:text-sm text-slate-500">You are all caught up for this section.</p>
            </div>
          ) : (
                  ...
                  {!isCancelled && (
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-800/20 p-3 sm:p-4 rounded-xl border border-slate-700/40 mt-1">
                      ...
                      <div className="flex flex-row flex-wrap sm:flex-nowrap gap-2 w-full sm:w-auto">
                        ...
                        {/* 🚀 NEW: Cancel Button for Technician */}
                        {activeTab !== 'Completed' && (
        ...
        {/* ======================================= */}
        {/* 🚀 MODAL 2: CANCEL ORDER MODAL (NEW)    */}
        {/* ======================================= */}
