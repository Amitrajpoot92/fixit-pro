...
import { 
  ...
  LogOut, Wrench, Tags, BadgeIndianRupee, ShieldCheck, X 
} from 'lucide-react';
import { signOut } from 'firebase/auth';
import { auth } from '../../../firebase';

export default function TechSidebar({ isSidebarOpen, setIsSidebarOpen }) {
  ...
  const mainItems = [
    ...
    { name: 'My Pricing', icon: Tags, path: '/technician/pricing' }, 
  ];

  const financeItems = [
    { name: 'My Wallet & Earnings', icon: Wallet, path: '/technician/earnings' },
    { name: 'KYC Verification', icon: ShieldCheck, path: '/technician/kyc' },
  ];
  ...
  // 🚀 FIX: Ab tablet aur mobile dono par click karte hi 100% close hoga
  ...
  return (
    <>
      ...
      {/* 🚀 FIX: Mobile par -translate-x-full ke sath w-64 rakha hai taaki icons bahar na jhaankein */}
      <aside className={`fixed lg:relative inset-y-0 left-0 z-50 bg-slate-900 border-r border-slate-800 flex flex-col h-screen transition-all duration-300 ${
        ...
        <nav className="flex-1 py-6 px-3 space-y-6 overflow-y-auto overflow-x-hidden custom-scrollbar">
          ...
          <div className="space-y-2">
            {isSidebarOpen && <p className="text-xs font-bold text-slate-500 uppercase tracking-wider pl-3 mb-2">Main Menu</p>}
            {mainItems.map((item) => {
          ...
          <div className="space-y-2 relative pt-2">
            ...
            {isSidebarOpen && (
              <p className="text-xs font-bold text-emerald-400 uppercase tracking-wider pl-3 mb-2 flex items-center gap-2">
                <BadgeIndianRupee className="w-4 h-4 text-emerald-400" /> Payments & Wallet
              </p>
            )}
