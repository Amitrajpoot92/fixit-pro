...
export default function NewProducts({ navigation }) {
  ...
  useEffect(() => {
    const q = query(collection(db, 'new_products'));
  ...
  const renderProductCard = ({ item }) => {
    ...
    return (
      <TouchableOpacity 
        ...
        // Pura product object pass kar rahe hain detail screen ko
    ...
    <FlatList 
      ...
      ListEmptyComponent={
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyTitle}>No new products found</Text>
        </View>
      }
    />
  );
...
const styles = StyleSheet.create({
  ...
  conditionBadge: { backgroundColor: '#E0F2FE', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  conditionText: { color: '#0369A1', fontSize: 8, fontWeight: '900' },
